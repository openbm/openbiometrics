...
See [Events & Webhooks](/openbiometrics/api/events) for details on subscribing to these events.

## Camera-Based Usage

For real-time person detection on camera streams, use the [Video Analytics](/openbiometrics/api/video-analytics) endpoints to add cameras. Person detection runs automatically on each frame when the module is enabled.
